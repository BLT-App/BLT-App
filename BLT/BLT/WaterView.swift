...
class WaterView: UIView, RenderTimerDelegate {
    ...
    fileprivate func getHue(_ waveNum: Int) -> CGFloat {
        return CGFloat(202.0 / 360.0) + (0.001 * CGFloat(waveNum))
    }
    ...
    func drawWave() {
        ...
        for waveNum in 0 ..< numberOfWaves {
            ...
            let waveHue: CGFloat = ((35.0 / CGFloat(numberOfWaves)) * CGFloat(waveNum) + 40.0) / 100.0
            let waveAlpha: CGFloat = (0.85 / (CGFloat(numberOfWaves) + 1)) * (CGFloat(waveNum) + 1.0)
            UIColor(hue: getHue(waveNum), saturation: waveHue, brightness: 0.8, alpha: waveAlpha).setFill()
